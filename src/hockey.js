...
export async function getMatchsAujourdhui() {
  try {
    const response = await fetch(
      `https://corsproxy.io/?${encodeURIComponent(`${NHL_API}/schedule/2026-03-24`)}`
    );
...
export async function getStatsEquipe(equipeId) {
  try {
    const response = await fetch(
      `https://corsproxy.io/?${encodeURIComponent(`${NHL_API}/standings/now`)}`
    );
    const data = await response.json();
...
export async function getScoreEnDirect(gameId) {
  try {
    const response = await fetch(
      `https://corsproxy.io/?${encodeURIComponent(`${NHL_API}/gamecenter/${gameId}/landing`)}`
    );
    const data = await response.json();
    return {
